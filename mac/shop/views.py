...
def index(request):
    ...
    cats = {items['subcategory'] for items in catprods}
    for cat in cats:
...
def searchMatch(query, item):
    if query in item.desc.lower() or query in item.category.lower() or query in item.subcategory.lower() or query in item.product_name.lower() :
        return True
    else :
        return False
